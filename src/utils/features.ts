...
export const translateWords = async (params: LangType): Promise<WordType[]> => {
  try {
    const wordsArray = generate(8);
    if (!Array.isArray(wordsArray)) {
      throw new Error("Expected generate to return an array of strings");
    }

    const words = wordsArray.map((i) => ({
      Text: i,
    }));
    ...
    const receive: FetchedDataType[] = response.data as FetchedDataType[];
